...
#load dataset
n_train=40000
n_valid=range(n_train,42000)
X_train0=np.array(pd.read_csv("train.csv"))[range(n_train),1:]
Y_train_array=np.array(pd.read_csv("train.csv"))[range(n_train),0]
X_valid0=np.array(pd.read_csv("train.csv"))[range(n_train,42000),1:]
Y_valid_array=np.array(pd.read_csv("train.csv"))[range(n_train,42000),0]
#number of features
p0=X_train0.shape[1]
...
X_valid=np.zeros((X_valid0.shape[0],p))
for i in range(p):
    j=np.int(X_features[i])
    X_train[:,i]=(X_train0[:,j]-X_mean[j,])/X_sd[j,]
    X_valid[:,i]=(X_valid0[:,j]-X_mean[j,])/X_sd[j,]
...
#convert output array to 0,1 matrix
Y_valid=np.zeros((Y_valid_array.shape[0],K))
for i in range(Y_valid_array.shape[0]):
    Y_valid[i,Y_valid_array[i]]=1
...
#threshold for improvement of the error over the validation_freq. If exceeded, adjust learning rate downward
...
update_param=[]
#gain parameter list of ones
gain=[]
gain_inc=np.array([0.05,0.95])
gain_min=0.01
gain_max=10.0
...
for layer in range(n_layers+1):
    ...
    gain.append(np.ones(alpha[layer].shape))
    update_old.append(np.zeros(alpha[layer].shape))
##################################################################################################
##################################################################################################
##################################################################################################
#TRAIN NETWORK
...
while (epoch_iter<max_epochs):
    ...
    for batch_number in range(n_mini_batch):
        ...
        for i in range(n_layers+1):
            ...
            update_param[i][M_range,:]=mom_rate*update_param[i][M_range,:]-eps_alpha*(grad[i][M_range,:]*gain[i][M_range,:]+eps_penalty*alpha[i][M_range,:])
            #apply smoothed update
            alpha[i][M_range,:]+=update_param[i][M_range,:]
            #no regularization for bias parameters
            update_param[i][0,:]=mom_rate*update_param[i][0,:]-eps_alpha*grad[i][0,:]*gain[i][0,:]
            #apply smoothed update
            alpha[i][0,:]+=update_param[i][0,:]

        #update the gain matrix for each layer's parameters
        if epoch_iter>0:
            for i in range(n_layers+1):
                gain[i]=(grad[i]*update_old[i]>=0.0)*(gain[i]+gain_inc[0])+(grad[i]*update_old[i]<0.0)*(gain[i]*gain_inc[1])
                gain[i]=np.maximum(gain[i],gain_min)
                gain[i]=np.minimum(gain[i],gain_max)

         #save the updates for the mini-batch
        for i in range(n_layers+1):
            #take minus sign since the gradient enters the update with a minus sign
            update_old[i]=-np.copy(update_param[i])
    ...
    y_pred_train=NN_classifier(alpha,X_train,special.expit,softmax_fn)
    ...
    if (epoch_iter>=min_epochs) and (epoch_iter % validation_freq==0):
        ...
        if avg_loss>loss_threshold:
            eps_alpha*=eps_adj
            #adjust gain to 1 for all weights
            for i in range(n_layers+1):
                gain[i]=np.ones(alpha[i].shape)
        ...
        y_pred_valid=NN_classifier(alpha,X_valid,special.expit,softmax_fn)
        y_pred_valid=y_pred_valid.argmax(1)+1
        #convert class matrix to array of class labels (starting at 1) for use in confusion matrix
        y_dat_valid=Y_valid.argmax(1)+1
        #compute confusion matrix using predicted outputs (y_pred) and actual labels (y_dat)
        CF=confusion_matrix_multi(y_pred_valid,y_dat_valid,K)
        # print CF
        accuracy=CF.diagonal().sum(0)/y_pred_valid.shape[0]
        print "validation set accuracy rate %s" %accuracy
##############################################################################################
##############################################################################################
##############################################################################################
X_test=np.column_stack((np.ones(X_test.shape[0]),X_test))
y_pred_test=NN_classifier(alpha,X_test,special.expit,softmax_fn)
