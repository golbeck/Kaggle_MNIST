...
def MLP_stoch_grad_mom(min_epochs,max_epochs,improvement_threshold,validation_freq,prob_dropout,batch_size,mom_param,gamma,eps_alpha,eps_penalty,alpha,M,X_train,Y_train,activation_fn,output_fn,grad_activation_fn,grad_output_fn):
...
parameters=MLP_stoch_grad_mom(min_epochs,max_epochs,improvement_threshold,validation_freq,prob_dropout,batch_size,mom_param,gamma,eps_alpha,eps_penalty,alpha,M,X_train,Y_train,special.expit,softmax_fn,grad_sigmoid,grad_softmax)
X_test=np.column_stack((np.ones(X_test.shape[0]),X_test))
y_pred_test=NN_classifier(parameters,X_test,special.expit,softmax_fn)
y_pred_test=y_pred_test.argmax(1).reshape((y_pred_test.shape[0],1))
y_out=np.column_stack((np.array(range(1,28001)),y_pred_test))
...
df.head(10)
df.to_csv("test_predictions800x800.csv",sep=",",index=False)